...
export function SectionInspector() {
  ...
  if (!section) {
    return (
      <aside
        ...
        className="order-2 hidden flex-col min-h-0 items-center justify-center gap-3 border-l border-border/50 bg-muted/30 p-8 text-center text-muted-foreground lg:order-3 lg:flex"
      >
  ...
  return (
    <aside aria-label="Section properties" className="order-2 flex flex-col min-h-0 border-l border-border/50 bg-muted/30 lg:order-3">
          ...
          <div key={field.path} className="space-y-1.5">
            ...
            {field.kind === "textarea" ? (
              ...
              <select
                ...
                aria-describedby={describedBy}
                onChange={(e) => setValue(field, e.target.value)}
                className="flex h-11 w-full rounded-xl border border-input bg-background px-4 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
              >
