    ...
    <div className="relative sm:hidden">
      ...
      {open ? (
        <div
          ref={panelRef}
          id="mobile-nav-panel"
          className="absolute right-0 top-full z-50 mt-2 w-44 overflow-hidden rounded-2xl border bg-popover/95 p-1.5 text-popover-foreground shadow-2xl backdrop-blur-xl animate-fade-in"
        >
          {LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="block rounded-md px-3 py-2 text-sm font-heading font-semibold hover:bg-muted focus-visible:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              {link.label}
            </Link>
          ))}
